  ...
  Future<void> _updateProfile(BuildContext context, fb.User user) async {
    ...
    try {
      ...
      if (_profileName.trim().isNotEmpty && _profileName.trim() != user.displayName) {
        ...
        await activeDb.collection('users').doc(user.uid).update({
          'name': _profileName.trim(),
        }).catchError((_) {});
    ...
    return div(classes: 'flex-1 p-6 md:p-8 flex flex-col gap-6 max-w-7xl mx-auto w-full bg-[#eff2f0]', [
      ...
      div(classes: 'flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-200/50 pb-5', [
        div(classes: 'flex flex-col gap-1', [
          h1(classes: 'text-xl font-black tracking-tight text-zinc-900', [text('System Configuration Console')]),
          p(classes: 'text-xs text-zinc-400 font-medium', [text('Audit controls for staff roles and platform threshold constraints.')]),
        ]),
      ]),
      ...
      div(classes: 'grid grid-cols-1 lg:grid-cols-2 gap-8', [
        ...
        div(classes: 'flex flex-col gap-8', [
          ...
          div(classes: 'p-6 rounded-[28px] border border-zinc-200/50 bg-white flex flex-col gap-4 shadow-[0_8px_30px_rgba(0,0,0,0.015)]', [
            h4(classes: 'text-sm font-bold text-zinc-900 flex items-center gap-2 border-b border-zinc-50 pb-3', [
              span([text('⚡')]),
              text('Feature Constraints (${currentEnv.name.toUpperCase()})')
            ]),
            p(classes: 'text-xs text-zinc-500 leading-relaxed font-medium', [
              text('System configuration parameters are loaded from public config documents. Modify threshold constraints below for reference.')
            ]),
            div(classes: 'flex flex-col gap-3 mt-1', [
              div(classes: 'flex items-center justify-between text-xs', [
                span(classes: 'text-zinc-500 font-semibold', [text('Daily Volume Cap')]),
                span(classes: 'font-extrabold text-zinc-900', [
                  text(currentEnv == Environment.production ? '₱500,000.00' : '₱50,000.00')
                ])
              ]),
              div(classes: 'flex items-center justify-between text-xs border-t border-zinc-100 pt-3', [
                span(classes: 'text-zinc-500 font-semibold', [text('Auto-Lockout Threshold')]),
                span(classes: 'font-extrabold text-zinc-900', [
                  text(currentEnv == Environment.production ? '5 verification failures' : '3 verification failures')
                ])
              ]),
              div(classes: 'flex items-center justify-between text-xs border-t border-zinc-100 pt-3', [
                span(classes: 'text-zinc-500 font-semibold', [text('Supported ID Cards')]),
                span(classes: 'font-extrabold text-indigo-500', [
                  text('Drivers License, Passport, UMID')
                ])
              ])
            ])
          ]),
          ...
          if (user != null)
            div(classes: 'p-6 rounded-[28px] border border-zinc-200/50 bg-white flex flex-col gap-4 shadow-[0_8px_30px_rgba(0,0,0,0.015)]', [
              h4(classes: 'text-sm font-bold text-zinc-900 flex items-center gap-2 border-b border-zinc-50 pb-3', [
                span([text('👤')]),
                text('Admin Profile Settings')
              ]),
              p(classes: 'text-xs text-zinc-500 font-medium leading-relaxed', [
                text('Update your account display name or reset your admin access password.')
              ]),

              if (_profileMessage != null)
                div(classes: 'p-3 rounded-xl bg-emerald-50 border border-emerald-200 text-[#0fa958] text-xs font-semibold', [
              ...
              div(classes: 'flex flex-col gap-3.5', [
                div(classes: 'flex flex-col gap-1.5', [
                  label(classes: 'text-[10px] text-zinc-400 font-bold uppercase tracking-wider', [text('Display Name')]),
                  input(
                    value: _profileName,
                ...
                div(classes: 'flex flex-col gap-1.5', [
                  label(classes: 'text-[10px] text-zinc-400 font-bold uppercase tracking-wider', [text('New Password')]),
                  input(
                    value: _newPassword,
                    classes: 'bg-[#f8faf9] border border-zinc-200/50 rounded-xl px-4 py-2.5 text-xs text-zinc-800 focus:outline-none focus:border-black focus:ring-1 focus:ring-black transition-all',
        ...
        if (isAdmin)
          div(classes: 'p-6 rounded-[28px] border border-zinc-200/50 bg-white flex flex-col gap-4 shadow-[0_8px_30px_rgba(0,0,0,0.015)]', [
            h4(classes: 'text-sm font-bold text-zinc-900 flex items-center gap-2 border-b border-zinc-50 pb-3', [
              span([text('👤')]),
              text('Register Staff / Customer Support Account')
            ]),
            p(classes: 'text-xs text-zinc-500 font-medium leading-relaxed', [
              text('Only admins can create support or management accounts. New users are assigned roles and recorded in the active environment.')
            ]),
            ...
            div(classes: 'flex flex-col gap-3.5', [
              div(classes: 'flex flex-col gap-1.5', [
                label(classes: 'text-[10px] text-zinc-400 font-bold uppercase tracking-wider', [text('Staff Name')]),
                input(
                  value: _newStaffName,
                  classes: 'bg-[#f8faf9] border border-zinc-200/50 rounded-xl px-4 py-2.5 text-xs text-zinc-800 focus:outline-none focus:border-black focus:ring-1 focus:ring-black transition-all',
              ...
              div(classes: 'flex flex-col gap-1.5', [
                label(classes: 'text-[10px] text-zinc-400 font-bold uppercase tracking-wider', [text('Staff Email')]),
                input(
              ...
              div(classes: 'flex flex-col gap-1.5', [
                label(classes: 'text-[10px] text-zinc-400 font-bold uppercase tracking-wider', [text('Staff Password')]),
                input(
              ...
              div(classes: 'flex flex-col gap-1.5', [
                label(classes: 'text-[10px] text-zinc-400 font-bold uppercase tracking-wider', [text('Access Role')]),
                select(
                  ...
                  onChange: (value) => _newStaffRole = (value as List<String>).isNotEmpty ? (value as List<String>).first : 'staff',
                  [
                    option(value: 'staff', selected: _newStaffRole == 'staff', [text('Staff (Customer Service)')]),
                    option(value: 'admin', selected: _newStaffRole == 'admin', [text('Admin Manager')]),
                  ]
                )
              ]),
          ...
          div(classes: 'p-6 rounded-[28px] border border-zinc-200/50 bg-white flex flex-col gap-4 shadow-[0_8px_30px_rgba(0,0,0,0.015)]', [
            h4(classes: 'text-sm font-bold text-zinc-900 flex items-center gap-2 border-b border-zinc-50 pb-3', [
              span([text('🔒')]),
              text('Access Privilege Policy')
            ]),
            p(classes: 'text-xs text-zinc-500 leading-relaxed font-medium', [
              text('Admins can view and escalate staff access privileges. Registered accounts default to support level. Audit configurations are logged.')
            ]),
            div(classes: 'flex flex-col gap-2 mt-2', [
              div(classes: 'p-4 rounded-xl bg-[#f8faf9] border border-zinc-200/50 flex justify-between items-center text-xs', [
                span(classes: 'font-semibold text-zinc-600', [text('Security Rules Status')]),
                span(classes: 'text-[#0fa958] font-extrabold text-[10px] tracking-wider uppercase', [text('Active')])
              ]),
              div(classes: 'p-4 rounded-xl bg-[#f8faf9] border border-zinc-200/50 flex justify-between items-center text-xs', [
                span(classes: 'font-semibold text-zinc-600', [text('Audited Logs Sync')]),
                span(classes: 'text-indigo-500 font-extrabold text-[10px] tracking-wider uppercase', [text('Streaming')])
              ])
            ])
          ])
      ])
