...
final paginatedListingsProvider = FutureProvider<List<ListingData>>((ref) async {
  ...
  return snap.docs.map((doc) {
    ...
    final reportsCount = data['reportCount'] as int? ?? (data['reports'] != null ? (data['reports'] as List).length : 0);
...
class _ListingsPageState extends State<ListingsPage> {
  ...
  Component build(BuildContext context) {
    ...
    final qnaList = _detailsModalItem == null
        ? <QnaMessage>[]
        : (context.watch(listingQnaStreamProvider((
            activeTab == 'vehicles' ? 'rentals' : activeTab == 'properties' ? 'properties' : 'jobs',
    ...
    if (selectedGlobalId != null && _detailsModalItem?.id != selectedGlobalId) {
      ...
      final col = activeTab == 'vehicles' ? 'rentals' : activeTab == 'properties' ? 'properties' : 'jobs';
    ...
    Component buildTabButton(String label, String value) {
      ...
      return button(
        ...
        classes: 'px-5 py-2 text-xs font-bold transition-all duration-200 '
    ...
    return div(classes: 'flex-1 p-6 md:p-8 flex flex-col gap-6 max-w-7xl mx-auto w-full bg-[#eff2f0] relative', [
              ...
              [
                ...
                div(classes: 'flex items-start justify-between border-b border-zinc-100 pb-4', [
                  div(classes: 'flex items-center gap-3', [
                    span(classes: 'text-2xl p-2.5 bg-[#f3f6f4] border border-zinc-200/50 rounded-2xl', [text(_detailsModalItem!.icon)]),
                    div(classes: 'flex flex-col', [
                      h2(classes: 'text-base font-black text-zinc-900 leading-tight', [text(_detailsModalItem!.title)]),
                      span(classes: 'text-[9px] text-zinc-400 font-mono mt-0.5', [text('ID: ${_detailsModalItem!.id}')]),
                    ])
                  ]),
                  button(
                    onClick: () {
                ...
                div(classes: 'flex flex-col gap-4 text-xs text-zinc-700', [
                  ...
                  div(classes: 'grid grid-cols-2 gap-4', [
                    div(classes: 'bg-[#f8faf9] p-3.5 rounded-2xl border border-zinc-200/20', [
                      span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Rate Parameters')]),
                      p(classes: 'text-sm font-black text-zinc-900 mt-1', [text(_detailsModalItem!.priceLabel)])
                    ]),
                    div(classes: 'bg-[#f8faf9] p-3.5 rounded-2xl border border-zinc-200/20', [
                      span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Listing Status')]),
                      div(classes: 'mt-1.5', [
                        span(classes: 'px-3 py-1 rounded-full text-[9px] font-extrabold border '
                            '${_detailsModalItem!.status == 'Available' || _detailsModalItem!.status == 'Open' 
                                ? 'bg-[#e2f1e9] text-[#0fa958] border-emerald-500/10' 
                                : 'bg-zinc-100 text-zinc-500 border-zinc-200'}', [
                          text(_detailsModalItem!.status.toUpperCase())
                        ])
                      ])
                    ]),
                  ]),
                  ...
                  div(classes: 'grid grid-cols-2 gap-4', [
                    div(classes: 'flex flex-col gap-1', [
                      span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Owner / Publisher')]),
                      p(classes: 'font-bold text-zinc-900', [text(_detailsModalItem!.owner)]),
                      span(classes: 'text-[9px] text-zinc-400 font-mono', [
                        text('ID: ${_detailsModalItem!.rawData['hostId'] ?? _detailsModalItem!.rawData['creatorId'] ?? 'N/A'}')
                      ])
                    ]),
                    div(classes: 'flex flex-col gap-1', [
                      span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Geographic Location')]),
                      p(classes: 'font-bold text-zinc-900', [text(getDisplayLocation(_detailsModalItem!.rawData))]),
                    ]),
                  ]),
                  ...
                  () {
                    ...
                    return div(classes: 'flex flex-col gap-1.5 border-t border-zinc-100 pt-3', [
                      span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Uploaded Media Images')]),
                  ...
                  // Description
                  if (_detailsModalItem!.rawData['description'] != null && _detailsModalItem!.rawData['description'].toString().isNotEmpty)
                    div(classes: 'flex flex-col gap-1 border-t border-zinc-100 pt-3', [
                      span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Description Details')]),
                      p(classes: 'text-zinc-650 leading-relaxed font-medium mt-1 bg-zinc-50/50 p-3 rounded-xl border border-zinc-100', [
                        text(_detailsModalItem!.rawData['description'].toString())
                      ]),
                    ]),

                  // Specifications
                  div(classes: 'flex flex-col gap-2 border-t border-zinc-100 pt-3', [
                    span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Technical Specifications')]),
                    div(classes: 'grid grid-cols-2 gap-2 bg-[#f8faf9] p-3 rounded-xl border border-zinc-200/20 text-[11px] font-medium text-zinc-600', [
                      if (activeTab == 'vehicles') ...[
                        span([text('Body Type: ${(_detailsModalItem!.rawData['bodyType'] ?? 'Sedan').toUpperCase()}')]),
                        span([text('Transmission: ${(_detailsModalItem!.rawData['transmission'] ?? 'Automatic').toUpperCase()}')]),
                        span([text('Fuel Type: ${(_detailsModalItem!.rawData['fuelType'] ?? 'Gasoline').toUpperCase()}')]),
                        span([text('Year model: ${_detailsModalItem!.rawData['year'] ?? 'N/A'}')]),
                      ] else if (activeTab == 'properties') ...[
                        span([text('Type: ${(_detailsModalItem!.rawData['propertyType'] ?? 'Apartment').toUpperCase()}')]),
                        span([text('Bedrooms: ${_detailsModalItem!.rawData['bedrooms'] ?? 1}')]),
                        span([text('Bathrooms: ${_detailsModalItem!.rawData['bathrooms'] ?? 1}')]),
                        span([text('Max Guests: ${_detailsModalItem!.rawData['maxGuests'] ?? 2}')]),
                      ] else ...[
                        span([text('Category: ${(_detailsModalItem!.rawData['category'] ?? 'Gig').toUpperCase()}')]),
                        span([text('Pricing Type: ${(_detailsModalItem!.rawData['pricingType'] ?? 'Fixed').toUpperCase()}')]),
                        span([text('Complexity: ${(_detailsModalItem!.rawData['complexity'] ?? 'Medium').toUpperCase()}')]),
                      ],
                    ]),
                  ]),
                  ...
                  // Public Q&A Board
                  div(classes: 'flex flex-col gap-2 border-t border-zinc-100 pt-3', [
                    span(classes: 'text-[9px] font-black text-indigo-500 uppercase tracking-wider', [text('💬 Public Q&A Moderation Feed')]),
                    if (qnaList.isEmpty)
                      div(classes: 'py-3 text-center text-[10px] text-zinc-400 font-semibold', [text('No public inquiries yet.')])
                    else
                      div(classes: 'flex flex-col gap-2 max-h-52 overflow-y-auto pr-1 no-scrollbar', [
                        for (final qna in qnaList)
                          div(classes: 'p-3 bg-zinc-50 border border-zinc-200/50 rounded-2xl flex flex-col gap-1.5 text-[10px]', [
                            div(classes: 'flex justify-between items-start', [
                              div(classes: 'flex items-center gap-1.5', [
                                span(classes: 'font-extrabold text-zinc-800', [text(qna.senderName)]),
                                span(classes: 'text-zinc-400 text-[8px] font-mono', [
                                  text(qna.createdAt > 0 
                                      ? DateTime.fromMillisecondsSinceEpoch(qna.createdAt).toString().split('.').first 
                                      : 'Just now')
                                ]),
                              ]),
                              button(
                                onClick: () async {
                                  if (web.window.confirm('Delete this message permanently?')) {
                                    ...
                                    if (qna.id.startsWith('mock_')) {
                                      ...
                                      await firestore.collection(col).doc(_detailsModalItem!.id).collection('qna').doc(qna.id).delete();
                                    }
                                  }
                                },
                            ...
                            p(classes: 'text-zinc-650 font-medium', [text(qna.text)]),
                            if (qna.reply != null)
                              div(classes: 'pl-3 border-l border-indigo-400 flex flex-col gap-0.5 mt-1 text-[10px]', [
                                span(classes: 'font-extrabold text-indigo-500', [text('Admin Response:')]),
                                p(classes: 'text-zinc-600 font-medium', [text(qna.reply!)]),
                              ])
                            else
                                ...
                                button(
                                  onClick: () async {
                                    ...
                                    if (qna.id.startsWith('mock_')) {
                                      ...
                                      await firestore.collection(col).doc(_detailsModalItem!.id).collection('qna').doc(qna.id).update({
                                        'reply': repText,
                                      });
                                    }
                                  },
                  ...
                  if (_detailsModalItem!.reportCount > 0)
                    div(classes: 'flex flex-col gap-2 border-t border-zinc-100 pt-3', [
                      span(classes: 'text-[9px] font-black text-red-500 uppercase tracking-wider', [text('🚨 Abuse Reports Logs')]),
                      div(classes: 'flex flex-col gap-1.5', [
                        for (final r in _detailsModalItem!.reports)
                          div(classes: 'p-2.5 bg-red-50/50 border border-red-200/30 rounded-xl flex flex-col gap-0.5 text-[10px]', [
                            p(classes: 'text-red-600 font-bold', [text('Reason: ${r is Map ? r['reason'] : r}')]),
                            if (r is Map && r['reporterName'] != null)
                              span(classes: 'text-zinc-400 font-medium', [text('Reporter: ${r['reporterName']}')])
                          ])
                      ])
                    ]),
                ]),
      ...
      div(classes: 'flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-200/50 pb-5', [
        div(classes: 'flex flex-col gap-1', [
          h1(classes: 'text-xl font-black tracking-tight text-zinc-900', [text('Listings Directory')]),
          p(classes: 'text-xs text-zinc-400 font-medium', [text('Review active services, vehicle rentals, and property listings.')]),
        ]),
      ...
      div(classes: 'w-full bg-white border border-zinc-200/50 rounded-[24px] p-5 shadow-[0_4px_20px_rgba(0,0,0,0.01)] flex flex-col gap-4', [
        ...
        div(classes: 'grid grid-cols-1 md:grid-cols-5 gap-3.5', [
          // Search Input
          div(classes: 'flex flex-col gap-1.5', [
            label(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wide', [text('Keyword Match')]),
            input(
              value: _searchQuery,
          ...
          // Location Input
          div(classes: 'flex flex-col gap-1.5', [
            label(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wide', [text('Location')]),
            input(
          ...
          div(classes: 'flex flex-col gap-1.5', [
            label(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wide', [
              text(activeTab == 'vehicles' ? 'Vehicle Type' : activeTab == 'properties' ? 'Property Type' : 'Service Type')
            ]),
            select(
              classes: 'px-4 py-2.5 bg-[#f3f6f4] border border-zinc-200/50 rounded-xl text-xs text-zinc-900 focus:outline-none focus:ring-1 focus:ring-black/10',
              onChange: (v) => setState(() => _categoryFilter = v.isNotEmpty ? v.first : ''),
              [
                option(value: '', selected: _categoryFilter == '', [text('ALL CATEGORIES')]),
                for (final opt in getCategoryOptions())
                  option(value: opt.toLowerCase(), selected: _categoryFilter == opt.toLowerCase(), [text(opt.toUpperCase())]),
              ]
            ),
          ]),
          // Min Price
          div(classes: 'flex flex-col gap-1.5', [
            label(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wide', [text('Min Rate (₱)')]),
            input(
          ...
          // Max Price
          div(classes: 'flex flex-col gap-1.5', [
            label(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wide', [text('Max Rate (₱)')]),
            input(
      ...
      listingsAsync.when(
        data: (listings) {
          ...
          if (filteredListings.isEmpty) {
            return div(classes: 'flex-grow flex flex-col items-center justify-center text-center p-16 bg-white border border-zinc-200/50 rounded-[28px] shadow-sm', [
              span(classes: 'text-3xl mb-3', [text('📂')]),
              h3(classes: 'text-sm font-bold text-zinc-900', [text('No listings match criteria')]),
              p(classes: 'text-xs text-zinc-500 mt-1', [text('Adjust or clear your active query filters above.')])
            ]);
          }

          return div(classes: 'flex flex-col gap-6', [
            div(classes: 'overflow-x-auto w-full rounded-[28px] border border-zinc-200/50 bg-white shadow-[0_8px_30px_rgba(0,0,0,0.015)]', [
              table(classes: 'w-full text-left text-xs border-collapse', [
                thead(classes: 'bg-[#f8faf9] text-zinc-500 font-bold border-b border-zinc-100 text-[10px] uppercase tracking-wider', [
                  tr([
                    th(classes: 'p-5', [text('Item / Service')]),
                    th(classes: 'p-5', [text('Publisher')]),
                    th(classes: 'p-5', [text('Location')]),
                    th(classes: 'p-5', [text('Rate Parameters')]),
                    th(classes: 'p-5 text-center', [text('Status')]),
                    th(classes: 'p-5 text-center', [text('Reports')]),
                    th(classes: 'p-5 text-center', [text('Created')]),
                    ...
                    tr(
                      ...
                      [
                        td(classes: 'p-5 font-bold text-zinc-900 flex items-center gap-3', [
                          span(classes: 'text-lg p-2 bg-[#f3f6f4] border border-zinc-200/50 rounded-xl', [text(item.icon)]),
                          div(classes: 'flex flex-col gap-0.5 min-w-0', [
                            span(classes: 'truncate font-bold text-zinc-805 hover:text-indigo-600 transition-colors', [text(item.title)]),
                            span(classes: 'text-[9px] text-zinc-400 font-mono', [text('ID: ${item.id}')]),
                          ])
                        ]),
                        td(classes: 'p-5 text-zinc-650 font-medium', [text(item.owner)]),
                        td(classes: 'p-5 text-zinc-650 font-semibold', [text(getDisplayLocation(item.rawData))]),
                        td(classes: 'p-5 font-extrabold text-zinc-900', [text(item.priceLabel)]),
                        ...
                        td(classes: 'p-5 text-center', [
                          if (item.reportCount > 0)
                            span(
                              ...
                              [text('🚩 ${item.reportCount} Reports')]
                            )
                          else
                            span(classes: 'text-zinc-400 font-semibold', [text('None')])
                        ]),
            ...
            div(classes: 'flex justify-between items-center px-4', [
              span(classes: 'text-[11px] text-zinc-400 font-semibold', [
              ...
              div(classes: 'flex items-center gap-2', [
                button(
                  ...
                  classes: 'px-4 py-2 rounded-full border border-zinc-200 text-[10px] font-extrabold tracking-wide uppercase transition-all duration-200 '
        ...
        loading: () => div(classes: 'flex-grow flex justify-center items-center py-20 bg-white border border-zinc-200/50 rounded-[28px] shadow-sm', [
          div(classes: 'animate-spin h-6 w-6 border-2 border-zinc-200 border-t-indigo-500 rounded-full', [])
        ]),
        error: (err, _) => div(classes: 'p-6 bg-red-50/5 border border-red-500/10 text-red-500 text-xs rounded-[20px] font-mono shadow-sm', [
