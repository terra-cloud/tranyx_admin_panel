  ...
  return firestore.collection('transactions').snapshots().map((snap) {
    ...
    for (final doc in snap.docs) {
      ...
      if (type == 'withdraw') {
        ...
        final isRental = data.containsKey('renteeId') || 
        ...
        if (isRental) {
          ...
          if (data.containsKey('totalFees')) {
            ...
            docRev = ((data['employerFees'] as num?)?.toDouble() ?? 0.0) + 
...
final userBreakdownProvider = StreamProvider<UserBreakdown>((ref) {
  ...
  final rentalsSub = firestore.collection('rentals').snapshots().listen((snap) {
    landlordIds = landlordIds.union(snap.docs.map((d) => (d.data()['hostId'] ?? '').toString()).where((id) => id.isNotEmpty).toSet());
    tryEmit();
  });
  ...
  final rentalReqSub = firestore.collection('rental_requests').snapshots().listen((snap) {
    renterIds = renterIds.union(snap.docs.map((d) => (d.data()['renteeId'] ?? '').toString()).where((id) => id.isNotEmpty).toSet());
    tryEmit();
  });
...
final liveListingsProvider = StreamProvider<LiveListingsBreakdown>((ref) {
  ...
  void tryEmit() {
    if (!controller.isClosed) {
      controller.add(LiveListingsBreakdown(
...
final reportedListingsCountProvider = StreamProvider<int>((ref) {
  ...
  final jobsSub = firestore.collection('jobs').snapshots().listen((snap) {
    jobsRep = snap.docs.where((d) => (d.data()['reportCount'] as int? ?? 0) > 0 || (d.data()['reports'] as List? ?? []).isNotEmpty).length;
    tryEmit();
  });

  final rentalsSub = firestore.collection('rentals').snapshots().listen((snap) {
    rentalsRep = snap.docs.where((d) => (d.data()['reportCount'] as int? ?? 0) > 0 || (d.data()['reports'] as List? ?? []).isNotEmpty).length;
    tryEmit();
  });

  final propertiesSub = firestore.collection('properties').snapshots().listen((snap) {
    propertiesRep = snap.docs.where((d) => (d.data()['reportCount'] as int? ?? 0) > 0 || (d.data()['reports'] as List? ?? []).isNotEmpty).length;
    tryEmit();
  });
...
final newListings24hProvider = StreamProvider<NewListingsStats>((ref) {
  ...
  void tryEmit() {
    if (vehicles != null && realEstate != null && services != null) {
      if (!controller.isClosed) {
        controller.add(NewListingsStats(
          vehicles: vehicles!,
  ...
  firestore.collection('rentals').where('createdAt', isGreaterThanOrEqualTo: cutoff).snapshots().listen(
        (s) { vehicles = s.docs.length; tryEmit(); },
        onError: (_) { vehicles = 0; tryEmit(); },
      );
  firestore.collection('properties').where('createdAt', isGreaterThanOrEqualTo: cutoff).snapshots().listen(
        (s) { realEstate = s.docs.length; tryEmit(); },
        onError: (_) { realEstate = 0; tryEmit(); },
      );
  firestore.collection('jobs').where('createdAt', isGreaterThanOrEqualTo: cutoff).snapshots().listen(
        (s) { services = s.docs.length; tryEmit(); },
        onError: (_) { services = 0; tryEmit(); },
      );
...
final kycStatsProvider = StreamProvider<KycStats>((ref) {
  final firestore = ref.watch(firestoreProvider);
  return firestore.collection('kyc_submissions').snapshots().map((snap) {
...
class SearchResult {
  ...
  SearchResult({required this.id, required this.title, required this.subtitle, required this.category, required this.path});
}
...
final globalSearchResultsProvider = FutureProvider.family<List<SearchResult>, String>((ref, query) async {
  ...
  try {
    ...
    for (final doc in fetches[0].docs) {
      final d = doc.data();
      if ((d['name'] as String? ?? '').toLowerCase().contains(q) || (d['email'] as String? ?? '').toLowerCase().contains(q)) {
        list.add(SearchResult(id: doc.id, title: d['name'] ?? 'User', subtitle: d['email'] ?? '', category: 'User', path: '/users'));
      }
    }
    // Rentals (Vehicles)
    for (final doc in fetches[1].docs) {
      ...
      if ('${d['brand']} ${d['model']}'.toLowerCase().contains(q)) {
        list.add(SearchResult(id: doc.id, title: '${d['brand']} ${d['model']}'.trim(), subtitle: 'Vehicle Rental', category: 'Vehicle', path: '/listings'));
      }
    }
    // Properties
    for (final doc in fetches[2].docs) {
      ...
      if ((d['title'] as String? ?? '').toLowerCase().contains(q)) {
        list.add(SearchResult(id: doc.id, title: d['title'] ?? 'Property', subtitle: 'Real Estate', category: 'Property', path: '/listings'));
      }
    }
    // Jobs (Services)
    for (final doc in fetches[3].docs) {
      final d = doc.data();
      if ((d['title'] as String? ?? '').toLowerCase().contains(q)) {
        list.add(SearchResult(id: doc.id, title: d['title'] ?? 'Service', subtitle: 'Gig Service', category: 'Service', path: '/listings'));
      }
    }
    ...
    for (final doc in fetches[4].docs) {
      ...
      if (title.toLowerCase().contains(q) || (d['renteeName'] as String? ?? '').toLowerCase().contains(q)) {
        list.add(SearchResult(id: doc.id, title: title, subtitle: 'Renter: ${d['renteeName'] ?? 'Guest'}', category: 'Booking: Vehicle', path: '/bookings'));
      }
    }
    ...
    for (final doc in fetches[5].docs) {
      ...
      if (title.toLowerCase().contains(q) || (d['renteeName'] as String? ?? '').toLowerCase().contains(q)) {
        list.add(SearchResult(id: doc.id, title: title, subtitle: 'Renter: ${d['renteeName'] ?? 'Guest'}', category: 'Booking: Property', path: '/bookings'));
      }
    }
  } catch (_) {}
...
class _DashboardState extends State<Dashboard> {
  ...
  Component build(BuildContext context) {
    ...
    final userBreakdown = context.watch(userBreakdownProvider).value ?? UserBreakdown(total: 0, jobSeekers: 0, employers: 0, landlords: 0, renters: 0);
    ...
    return div(classes: 'flex-1 p-6 md:p-8 flex flex-col gap-7 max-w-7xl mx-auto w-full bg-[#f2f5f3]', [
      ...
      div(classes: 'flex flex-col md:flex-row md:items-center justify-between gap-4 pb-1', [
        div(classes: 'flex flex-col gap-1', [
          span(classes: 'text-[10px] text-zinc-400 font-bold uppercase tracking-wider', [text(dateString)]),
          h1(classes: 'text-2xl font-black text-zinc-900 tracking-tight', [text('Analytics Dashboard')]),
          p(classes: 'text-xs text-zinc-400 font-bold', [
            text('Live platform metrics — ${currentEnv.name.toUpperCase()} environment'),
          ]),
        ]),
        div(classes: 'flex items-center gap-3 self-end md:self-auto', [
          div(classes: 'relative hidden sm:block', [
            ...
            if (_searchQuery.isNotEmpty)
              div(classes: 'absolute top-11 left-0 bg-white border border-zinc-200/80 rounded-2xl shadow-xl z-50 max-h-80 overflow-y-auto p-3 flex flex-col gap-1 w-72', [
                context.watch(globalSearchResultsProvider(_searchQuery)).when(
                  data: (results) {
                    if (results.isEmpty) return div(classes: 'text-center p-4 text-[11px] text-zinc-400', [text('No results found')]);
                    return fragment([
                      for (final res in results)
                        a(
                          href: 'javascript:void(0);',
                          onClick: () {
                            if (res.category == 'Vehicle' || res.category == 'Property' || res.category == 'Service') {
                          ...
                          [
                            div(classes: 'flex justify-between items-center', [
                              span(classes: 'text-[11px] font-black text-zinc-800 truncate max-w-[70%]', [text(res.title)]),
                              span(classes: 'text-[8px] font-extrabold uppercase px-1.5 py-0.5 rounded bg-zinc-100 text-zinc-500 tracking-wider', [text(res.category)]),
                            ]),
                          ],
                        ),
                    ]);
                  },
                  loading: () => div(classes: 'flex justify-center p-4', [div(classes: 'animate-spin h-4 w-4 border-2 border-zinc-200 border-t-black rounded-full', [])]),
                  error: (_, __) => div(classes: 'text-center p-3 text-[10px] text-red-500', [text('Search error')]),
                ),
              ]),
          ]),
          div(classes: 'flex items-center gap-2.5 bg-white px-3 py-1.5 border border-zinc-200/50 rounded-full shadow-sm', [
            div(classes: 'w-7 h-7 rounded-full bg-zinc-200 flex items-center justify-center font-black text-xs text-zinc-700', [
              text(user?.displayName?.isNotEmpty == true ? user!.displayName!.substring(0, 1).toUpperCase() : 'A'),
            ]),
            div(classes: 'flex flex-col text-left leading-tight hidden sm:flex', [
              span(classes: 'text-[10px] font-black text-zinc-850', [text(user?.displayName ?? 'Admin')]),
              span(classes: 'text-[8px] text-zinc-400 font-extrabold uppercase', [text('Admin')]),
            ]),
          ]),
        ]),
      ]),
      ...
      div(classes: 'grid grid-cols-2 sm:grid-cols-4 gap-3', [
        ...
        _promoBanner('P2P Listings', 'Review platform listings', 'bg-emerald-50 border border-emerald-100', '/listings'),
        _promoBanner('System Config', 'Environment & node settings', 'bg-purple-50 border border-purple-100', '/settings'),
      ]),
      ...
      div(classes: 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4', [
        ...
        div(classes: 'bg-white rounded-[22px] border border-zinc-200/50 p-5 flex flex-col gap-3 shadow-[0_4px_20px_rgba(0,0,0,0.02)] lg:col-span-1', [
          div(classes: 'flex items-start justify-between', [
            div(classes: 'flex flex-col gap-0.5 min-w-0', [
              span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Total Revenue')]),
          ...
          div(classes: 'flex flex-col gap-1.5 pt-2 border-t border-zinc-100 text-[9px] font-semibold text-zinc-500', [
            div(classes: 'flex items-center justify-between', [
              span([text('Services (Employer 10%, Worker 3%)')]),
              span(classes: 'font-bold text-zinc-800', [
                text('₱${(() {
                  final bd = _revenueTimeframe == '7d' ? detailedRevenue.breakdown7d : _revenueTimeframe == '30d' ? detailedRevenue.breakdown30d : _revenueTimeframe == 'allTime' ? detailedRevenue.breakdownAllTime : detailedRevenue.breakdown24h;
                  return bd.services.toStringAsFixed(2);
                })()}'),
              ]),
            ]),
            div(classes: 'flex items-center justify-between', [
              span([text('Rentals (Lessor 3%, Lessee 3%, Listing 1.5%)')]),
              span(classes: 'font-bold text-zinc-800', [
                text('₱${(() {
                  final bd = _revenueTimeframe == '7d' ? detailedRevenue.breakdown7d : _revenueTimeframe == '30d' ? detailedRevenue.breakdown30d : _revenueTimeframe == 'allTime' ? detailedRevenue.breakdownAllTime : detailedRevenue.breakdown24h;
            ...
            div(classes: 'flex items-center justify-between', [
              span([text('Withdrawals (Workers 2% Platform Fee)')]),
              span(classes: 'font-bold text-zinc-800', [
                text('₱${(() {
                  final bd = _revenueTimeframe == '7d' ? detailedRevenue.breakdown7d : _revenueTimeframe == '30d' ? detailedRevenue.breakdown30d : _revenueTimeframe == 'allTime' ? detailedRevenue.breakdownAllTime : detailedRevenue.breakdown24h;
        ...
        div(classes: 'bg-white rounded-[22px] border border-zinc-200/50 p-5 flex flex-col gap-3 shadow-[0_4px_20px_rgba(0,0,0,0.02)]', [
          div(classes: 'flex items-start justify-between', [
            div(classes: 'flex flex-col gap-0.5', [
              span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Active Users')]),
              h4(classes: 'text-2xl font-black text-zinc-900 tracking-tight mt-1', [text(userBreakdown.total.toString())]),
            ]),
            div(classes: 'w-8 h-8 rounded-2xl flex items-center justify-center text-lg bg-indigo-50 text-indigo-500 flex-shrink-0', [text('👥')]),
          ]),
          div(classes: 'grid grid-cols-2 gap-1 pt-2 border-t border-zinc-100 text-[8px] font-bold text-zinc-500', [
            div(classes: 'flex flex-col', [
              span(classes: 'text-zinc-400 uppercase font-extrabold', [text('Seekers')]),
              span(classes: 'text-xs font-black text-zinc-800', [text(userBreakdown.jobSeekers.toString())]),
            ]),
            div(classes: 'flex flex-col', [
              span(classes: 'text-zinc-400 uppercase font-extrabold', [text('Employers')]),
              span(classes: 'text-xs font-black text-zinc-800', [text(userBreakdown.employers.toString())]),
            ]),
            div(classes: 'flex flex-col border-t border-zinc-50 pt-1', [
              span(classes: 'text-zinc-400 uppercase font-extrabold', [text('Landlords')]),
              span(classes: 'text-xs font-black text-zinc-800', [text(userBreakdown.landlords.toString())]),
            ]),
            div(classes: 'flex flex-col border-t border-zinc-50 pt-1', [
              span(classes: 'text-zinc-400 uppercase font-extrabold', [text('Renters')]),
              span(classes: 'text-xs font-black text-zinc-800', [text(userBreakdown.renters.toString())]),
            ]),
          ]),
        ]),
        ...
        div(classes: 'bg-white rounded-[22px] border border-zinc-200/50 p-5 flex flex-col gap-3 shadow-[0_4px_20px_rgba(0,0,0,0.02)]', [
          div(classes: 'flex items-start justify-between', [
            div(classes: 'flex flex-col gap-0.5', [
              span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Live Listings')]),
              h4(classes: 'text-2xl font-black text-zinc-900 tracking-tight mt-1', [text(liveListings.total.toString())]),
            ]),
            div(classes: 'w-8 h-8 rounded-2xl flex items-center justify-center text-lg bg-emerald-50 text-emerald-500 flex-shrink-0', [text('📋')]),
          ]),
          div(classes: 'grid grid-cols-3 gap-1 pt-2 border-t border-zinc-100 text-[8px] font-bold text-zinc-500 text-center', [
            div(classes: 'flex flex-col py-1 bg-amber-50 text-amber-700 rounded-lg', [
              span(classes: 'text-[7px] font-extrabold uppercase', [text('Vehicles')]),
              span(classes: 'text-xs font-black', [text(liveListings.vehicles.toString())]),
            ]),
            div(classes: 'flex flex-col py-1 bg-blue-50 text-blue-700 rounded-lg', [
              span(classes: 'text-[7px] font-extrabold uppercase', [text('Properties')]),
              span(classes: 'text-xs font-black', [text(liveListings.properties.toString())]),
            ]),
            div(classes: 'flex flex-col py-1 bg-purple-50 text-purple-700 rounded-lg', [
              span(classes: 'text-[7px] font-extrabold uppercase', [text('Services')]),
              span(classes: 'text-xs font-black', [text(liveListings.services.toString())]),
            ]),
          ]),
        ]),
        ...
        div(classes: 'bg-white rounded-[22px] border border-zinc-200/50 p-5 flex flex-col gap-3 shadow-[0_4px_20px_rgba(0,0,0,0.02)]', [
          div(classes: 'flex items-start justify-between', [
            div(classes: 'flex flex-col gap-0.5', [
              span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Pending KYC')]),
              h4(classes: 'text-2xl font-black text-amber-500 tracking-tight mt-1', [text(kycStats.pending.toString())]),
            ]),
            div(classes: 'w-8 h-8 rounded-2xl flex items-center justify-center text-lg bg-amber-50 text-amber-500 flex-shrink-0', [text('🪪')]),
          ]),
          div(classes: 'flex flex-col gap-1 pt-2 border-t border-zinc-100 text-[9px] font-semibold text-zinc-500', [
            div(classes: 'flex items-center justify-between', [
              span([text('Approved ID docs')]),
              span(classes: 'font-bold text-zinc-800', [text(kycStats.approved.toString())]),
            ]),
            div(classes: 'flex items-center justify-between', [
              span([text('Rejected ID docs')]),
              span(classes: 'font-bold text-zinc-800', [text(kycStats.rejected.toString())]),
            ]),
            if (reportedListingsCount > 0)
              div(classes: 'mt-1.5 px-2 py-1 bg-red-50 text-red-600 rounded-lg font-black text-[8px] flex items-center justify-between animate-pulse', [
                text('🚨 FLAG: $reportedListingsCount REPORTED POSTS'),
              ])
            else
              div(classes: 'mt-1.5 px-2 py-1 bg-emerald-50 text-emerald-600 rounded-lg font-black text-[8px]', [
                text('✓ NO URGENT POST ISSUES'),
              ]),
          ]),
        ]),
        ...
        div(classes: 'bg-white rounded-[22px] border border-zinc-200/50 p-5 flex flex-col gap-3 shadow-[0_4px_20px_rgba(0,0,0,0.02)]', [
          div(classes: 'flex items-start justify-between', [
            div(classes: 'flex flex-col gap-0.5', [
              span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Open Tickets / Reports')]),
              h4(classes: 'text-2xl font-black text-red-500 tracking-tight mt-1', [text((openTicketsCount + reportedListingsCount).toString())]),
            ]),
            div(classes: 'w-8 h-8 rounded-2xl flex items-center justify-center text-lg bg-red-50 text-red-500 flex-shrink-0', [text('🎟️')]),
          ]),
          div(classes: 'flex flex-col gap-1 pt-2 border-t border-zinc-100 text-[9px] font-semibold text-zinc-500', [
            div(classes: 'flex items-center justify-between', [
              span([text('Open Support Tickets')]),
              span(classes: 'font-bold text-zinc-800', [text(openTicketsCount.toString())]),
            ]),
            div(classes: 'flex items-center justify-between', [
              span([text('Reported Postings')]),
              span(classes: 'font-bold text-zinc-800', [text(reportedListingsCount.toString())]),
            ]),
            a(
              href: '/tickets',
              classes: 'mt-1.5 text-center text-[8px] font-black uppercase text-indigo-500 hover:text-indigo-600 transition-colors',
              [text('RESOLVE PENDING ISSUES →')]
            ),
          ]),
        ]),
      ...
      div(classes: 'grid grid-cols-1 lg:grid-cols-3 gap-5', [
        ...
        div(classes: 'lg:col-span-2 bg-white rounded-[28px] border border-zinc-200/50 p-6 flex flex-col gap-5 shadow-[0_8px_30px_rgba(0,0,0,0.01)]', [
          div(classes: 'flex justify-between items-center border-b border-zinc-50 pb-4', [
            div([
              h3(classes: 'text-sm font-black text-zinc-900', [text('Top Performing Agents')]),
              p(classes: 'text-[10px] text-zinc-400 font-bold mt-0.5', [text('Ranked by resolved support chats & CSAT score')]),
            ]),
            a(href: '/users', classes: 'text-[10px] font-extrabold text-zinc-500 hover:text-black transition-colors no-underline', [text('Manage →')]),
          ]),
          if (staffList.isEmpty)
            div(classes: 'py-8 text-center flex flex-col items-center gap-2', [
              span(classes: 'text-3xl', [text('🧑‍💼')]),
              span(classes: 'text-xs font-bold text-zinc-600', [text('No support agents yet')]),
              span(classes: 'text-[10px] text-zinc-400', [text('Add agents in Users → Agents / Staff tab.')]),
            ])
          else ...[
            div(classes: 'grid grid-cols-12 text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider px-1 pb-1', [
              div(classes: 'col-span-1', [text('#')]),
              div(classes: 'col-span-5', [text('Agent')]),
              div(classes: 'col-span-3 text-center', [text('Resolved')]),
              div(classes: 'col-span-3 text-center', [text('CSAT')]),
            ]),
            for (var i = 0; i < staffList.length && i < 8; i++)
              div(classes: 'grid grid-cols-12 items-center py-3 border-b border-zinc-50 last:border-0 hover:bg-zinc-50/50 rounded-xl px-1 transition-colors', [
                div(classes: 'col-span-1 flex justify-center', [
                  if (i == 0) span(classes: 'text-sm', [text('🥇')])
                  else if (i == 1) span(classes: 'text-sm', [text('🥈')])
                  else if (i == 2) span(classes: 'text-sm', [text('🥉')])
                  else span(classes: 'text-[10px] font-extrabold text-zinc-400', [text('${i + 1}')]),
                ]),
                div(classes: 'col-span-5 flex items-center gap-2.5', [
                  div(classes: 'w-8 h-8 rounded-full bg-indigo-50 border border-indigo-100 flex items-center justify-center font-extrabold text-zinc-700 text-[10px] flex-shrink-0', [
                    text(staffList[i].name.length >= 2 ? staffList[i].name.substring(0, 2).toUpperCase() : staffList[i].name.toUpperCase()),
                  ]),
                  div(classes: 'flex flex-col min-w-0', [
                    span(classes: 'text-xs font-black text-zinc-800 truncate', [text(staffList[i].name)]),
                    span(classes: 'text-[9px] text-indigo-500 font-extrabold uppercase', [text(staffList[i].role)]),
                  ]),
                ]),
                div(classes: 'col-span-3 text-center', [
                  span(classes: 'text-sm font-black text-zinc-900', [text(staffList[i].resolvedChats.toString())]),
                  span(classes: 'text-[9px] text-zinc-400 ml-0.5', [text(' chats')]),
                ]),
                div(classes: 'col-span-3 text-center', [
                  if (staffList[i].csat > 0)
                    span(classes: 'text-xs font-black ${staffList[i].csat >= 95 ? "text-[#0fa958]" : staffList[i].csat >= 90 ? "text-amber-500" : "text-red-500"}', [
                      text('${staffList[i].csat}%'),
                    ])
                  else
                    span(classes: 'text-[9px] text-zinc-400 font-bold', [text('N/A')]),
                ]),
              ]),
          ],
        ]),
        ...
        // Platform Config
        div(classes: 'bg-white rounded-[28px] border border-zinc-200/50 p-6 flex flex-col gap-5 shadow-[0_8px_30px_rgba(0,0,0,0.01)]', [
          div(classes: 'border-b border-zinc-50 pb-4', [
            h3(classes: 'text-sm font-black text-zinc-900', [text('Platform Config')]),
            p(classes: 'text-[10px] text-zinc-400 font-bold mt-0.5', [text('Node status and environment health')]),
          ]),
          div(classes: 'flex flex-col gap-4', [
            ...
            _configRow('Support Queue', '${(context.watch(dbStaffCountProvider).value ?? 0)} agents active', 'Live', '#f59e0b', '#fef3c7'),
          ]),
        ]),
      ]),
...
Component _promoBanner(String title, String desc, String cls, String path) {
  return a(
    href: path,
    classes: '$cls p-4 rounded-[20px] flex flex-col justify-between h-24 transition-all hover:scale-[1.02] no-underline shadow-sm',
    [
      span(classes: 'text-xs font-extrabold text-zinc-800', [text(title)]),
      span(classes: 'text-[10px] text-zinc-500 font-semibold leading-snug', [text(desc)]),
    ],
  );
}
...
Component _configRow(String label, String sub, String status, String color, String bg) {
  return div(classes: 'flex items-center justify-between', [
    div(classes: 'flex flex-col gap-0.5', [
      span(classes: 'text-xs font-bold text-zinc-800', [text(label)]),
      span(classes: 'text-[9px] text-zinc-400 font-bold', [text(sub)]),
    ]),
    span(classes: 'px-2 py-0.5 rounded-full text-[9px] font-extrabold border',
