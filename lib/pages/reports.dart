...
final reportedJobsStreamProvider = StreamProvider<List<ListingData>>((ref) {
  ...
  return firestore.collection('jobs').snapshots().map((snap) {
    ...
    }).where((item) => item.reportCount > 0).toList();
  });
});
  ...
  Component build(BuildContext context) {
    ...
    return div(classes: 'w-full p-8 flex flex-col gap-6 relative', [
      ...
      if (_detailsModalItem != null)
        div(
          classes: 'fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in',
          events: {
              ...
              [
                ...
                div(classes: 'flex items-center justify-between border-b border-zinc-50 pb-3', [
                  div(classes: 'flex items-center gap-2.5', [
                    span(classes: 'text-2xl', [text(_detailsModalItem!.icon)]),
                    div(classes: 'flex flex-col min-w-0', [
                      h3(classes: 'text-sm font-black text-zinc-900 truncate max-w-xs', [text(_detailsModalItem!.title)]),
                      span(classes: 'text-[9px] text-zinc-400 font-bold uppercase tracking-wider', [
                        text('${_detailsModalItem!.rawData['collectionType']?.toString().toUpperCase()} • ID: ${_detailsModalItem!.id.substring(0, 8)}')
                      ]),
                    ])
                  ]),
                  button(
                ...
                div(classes: 'flex flex-col gap-3.5 max-h-[360px] overflow-y-auto pr-1.5', [
                  div(classes: 'grid grid-cols-2 gap-3 text-xs', [
                    div(classes: 'bg-[#fafbfa] p-3 rounded-2xl border border-zinc-200/30', [
                      span(classes: 'text-[9px] font-black text-zinc-400 uppercase tracking-wider', [text('Publisher')]),
                      p(classes: 'font-black text-zinc-800 mt-0.5 truncate', [text(_detailsModalItem!.owner)]),
                    ]),
                    div(classes: 'bg-[#fafbfa] p-3 rounded-2xl border border-zinc-200/30', [
                      span(classes: 'text-[9px] font-black text-zinc-400 uppercase tracking-wider', [text('Pricing / Rate')]),
                      p(classes: 'font-black text-zinc-800 mt-0.5 truncate', [text(_detailsModalItem!.priceLabel)]),
                    ])
                  ]),
                  ...
                  if (_detailsModalItem!.rawData['description'] != null)
                    div(classes: 'flex flex-col gap-1', [
                      span(classes: 'text-[9px] font-black text-zinc-400 uppercase tracking-wider', [text('Description')]),
                      p(classes: 'text-[11px] text-zinc-600 bg-zinc-50/50 p-2.5 rounded-xl border border-zinc-100/50 leading-relaxed font-medium', [
                        text(_detailsModalItem!.rawData['description'] as String)
                      ])
                    ]),
                  ...
                  if (_detailsModalItem!.reportCount > 0)
                    div(classes: 'flex flex-col gap-2 border-t border-zinc-100 pt-3', [
                      span(classes: 'text-[9px] font-black text-red-500 uppercase tracking-wider', [text('🚨 Abuse Reports Logs (${_detailsModalItem!.reportCount})')]),
                      div(classes: 'flex flex-col gap-1.5', [
                        for (final r in _detailsModalItem!.reports)
                          div(classes: 'p-2.5 bg-red-50/50 border border-red-200/30 rounded-xl flex flex-col gap-0.5 text-[10px]', [
                            p(classes: 'text-red-650 font-bold', [text('Reason: ${r is Map ? r['reason'] : r}')]),
      ...
      div(classes: 'flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-200/50 pb-5', [
        div(classes: 'flex flex-col gap-1', [
          h1(classes: 'text-xl font-black tracking-tight text-zinc-900', [text('Abuse Reports Center')]),
          p(classes: 'text-xs text-zinc-400 font-medium', [text('Review flagged listings, view log reasons, and moderate accounts.')]),
        ]),
      ]),
      ...
      div(classes: 'w-full bg-white border border-zinc-200/50 rounded-[24px] p-5 shadow-[0_4px_20px_rgba(0,0,0,0.01)] flex flex-col gap-4', [
        div(classes: 'flex items-center justify-between', [
          h3(classes: 'text-[11px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Reports Filters')]),
        ...
        div(classes: 'grid grid-cols-1 md:grid-cols-3 gap-3.5', [
          ...
          // Listing Type
          div(classes: 'flex flex-col gap-1.5', [
            label(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wide', [text('Listing Type')]),
            select(
              ...
              [
                option(value: 'all', selected: _typeFilter == 'all', [text('ALL LISTINGS')]),
                option(value: 'services', selected: _typeFilter == 'services', [text('💼 SERVICES (GIGS)')]),
                option(value: 'vehicles', selected: _typeFilter == 'vehicles', [text('🚗 VEHICLES')]),
                option(value: 'properties', selected: _typeFilter == 'properties', [text('🏠 REAL ESTATE')]),
              ]
            ),
          ]),
        ]),
      ...
      reportedListingsAsync.when(
        data: (listings) {
          ...
          if (filtered.isEmpty) {
            return div(classes: 'w-full bg-white border border-zinc-200/50 rounded-[28px] p-12 text-center flex flex-col items-center justify-center gap-3.5 shadow-[0_4px_20px_rgba(0,0,0,0.01)]', [
              span(classes: 'text-4xl animate-bounce', [text('🛡️')]),
              h3(classes: 'text-sm font-black text-zinc-800', [text('Inbox is clean')]),
              p(classes: 'text-xs text-zinc-400 max-w-xs leading-relaxed', [
                text('No listings matching active filters have reported flags at the moment.')
              ])
            ]);
          }

          return div(classes: 'bg-white rounded-[28px] border border-zinc-200/50 shadow-[0_4px_25px_rgba(0,0,0,0.01)] overflow-hidden', [
            div(classes: 'overflow-x-auto', [
              table(classes: 'w-full border-collapse text-left text-xs', [
                thead(classes: 'bg-[#fafbfa] border-b border-zinc-150', [
                  tr([
                    th(classes: 'py-3.5 px-6 font-extrabold text-zinc-400 uppercase tracking-wider', [text('Type')]),
                    th(classes: 'py-3.5 px-6 font-extrabold text-zinc-400 uppercase tracking-wider', [text('Listing Title')]),
                    th(classes: 'py-3.5 px-6 font-extrabold text-zinc-400 uppercase tracking-wider', [text('Publisher')]),
                    th(classes: 'py-3.5 px-6 font-extrabold text-zinc-400 uppercase tracking-wider', [text('Flags')]),
                    th(classes: 'py-3.5 px-6 font-extrabold text-zinc-400 uppercase tracking-wider', [text('Report Log')]),
                    th(classes: 'py-3.5 px-6 font-extrabold text-zinc-400 uppercase tracking-wider text-right', [text('Moderation')]),
                  ])
                ]),
                tbody(classes: 'divide-y divide-zinc-100', [
                  for (final item in filtered)
                    tr(classes: 'hover:bg-zinc-50/40 transition-colors', [
                      td(classes: 'py-4 px-6 font-black text-zinc-800 text-lg', [text(item.icon)]),
                      td(classes: 'py-4 px-6 min-w-[200px]', [
                        span(classes: 'font-black text-zinc-900 block hover:underline cursor-pointer', 
                          events: {'click': (e) => setState(() => _detailsModalItem = item)},
                          [text(item.title)]
                        ),
                        span(classes: 'text-[9px] text-zinc-400 font-bold block mt-0.5', [text(item.id)])
                      ]),
                      td(classes: 'py-4 px-6 font-bold text-zinc-700', [text(item.owner)]),
                      td(classes: 'py-4 px-6', [
                        span(classes: 'px-2 py-0.5 bg-red-50 text-red-500 border border-red-200 text-[10px] font-black rounded-full', [
                          text('${item.reportCount} flags')
                        ])
                      ]),
                      td(classes: 'py-4 px-6 max-w-[240px] truncate font-medium text-zinc-500', [
                        text(item.reports.isNotEmpty
                            ? (item.reports.first is Map ? item.reports.first['reason']?.toString() ?? '' : item.reports.first.toString())
                            : 'No log details')
                      ]),
        ...
        error: (err, _) => div(classes: 'p-6 bg-red-50 text-red-500 rounded-2xl border border-red-200 font-mono text-xs', [
          text('Failed loading reports: $err')
        ])
      )
    ]);
  }
