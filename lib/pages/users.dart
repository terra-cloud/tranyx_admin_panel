...
final userTransactionsProvider = StreamProvider.family<List<TxModel>, String>((ref, uid) {
  ...
  return firestore
      ...
      .snapshots()
      .map((snap) => snap.docs
          .map((doc) => TxModel.fromMap(doc.id, doc.data()))
          .where((tx) => tx.uid == uid)
          .toList())
      .handleError((err) {
...
final userActivityHistoryProvider = StreamProvider.family<List<ActivityEvent>, String>((ref, uid) {
  ...
  final jobsSub = firestore.collection('jobs').where('creatorId', isEqualTo: uid).snapshots().listen((snap) {
    subEvents['jobs'] = snap.docs.map((d) => ActivityEvent(
  ...
  final rentalsSub = firestore.collection('rentals').where('hostId', isEqualTo: uid).snapshots().listen((snap) {
    subEvents['rentals'] = snap.docs.map((d) => ActivityEvent(
      type: 'listing_created',
  ...
  final propertiesSub = firestore.collection('properties').where('hostId', isEqualTo: uid).snapshots().listen((snap) {
    subEvents['properties'] = snap.docs.map((d) => ActivityEvent(
      type: 'listing_created',
  ...
  final rentalReqSub = firestore.collection('rental_requests').where('renteeId', isEqualTo: uid).snapshots().listen((snap) {
    subEvents['rental_requests'] = snap.docs.map((d) => ActivityEvent(
      type: 'booking_made',
      title: 'Booked Vehicle: "${d.data()['brand'] ?? ''} ${d.data()['model'] ?? ''}" (Status: ${d.data()['status'] ?? 'Pending'})',
      timestamp: getTimestamp(d.data()['createdAt']),
    )).toList();
    emitMerged();
  });

  final propertyReqSub = firestore.collection('property_requests').where('renteeId', isEqualTo: uid).snapshots().listen((snap) {
    subEvents['property_requests'] = snap.docs.map((d) => ActivityEvent(
      type: 'booking_made',
  ...
  final ticketsSub = firestore.collection('supportTickets').where('uid', isEqualTo: uid).snapshots().listen((snap) {
    subEvents['tickets'] = snap.docs.map((d) => ActivityEvent(
...
class _UsersPageState extends State<UsersPage> {
  ...
  Component build(BuildContext context) {
    ...
    return div(classes: 'flex-1 p-6 md:p-8 flex flex-col gap-8 max-w-7xl mx-auto w-full bg-[#eff2f0] relative', [
              ...
              [
                ...
                div(classes: 'flex items-start justify-between border-b border-zinc-100 pb-4', [
                  div(classes: 'flex items-center gap-3', [
                    div(classes: 'w-11 h-11 rounded-full bg-zinc-100 border border-zinc-200 flex items-center justify-center font-bold text-zinc-700 text-sm flex-shrink-0', [
                      text(_selectedUser!.name.length >= 2 ? _selectedUser!.name.substring(0, 2).toUpperCase() : _selectedUser!.name.toUpperCase())
                    ]),
                    div(classes: 'flex flex-col', [
                      h2(classes: 'text-base font-black text-zinc-950 leading-tight', [
                        text(_selectedUser!.name),
                        if (_selectedUser!.banned)
                          span(classes: 'ml-2 px-2 py-0.5 rounded bg-red-100 text-red-500 font-extrabold text-[8px] tracking-wider uppercase align-middle', [text('Banned')])
                      ]),
                      span(classes: 'text-[9px] text-zinc-400 font-mono mt-0.5', [text('UID: ${_selectedUser!.uid}')]),
                    ])
                  ]),
                  button(
                ...
                // Modal Tabs Selector
                div(classes: 'flex items-center gap-1 bg-zinc-50 p-1 border border-zinc-200/50 rounded-full shadow-inner w-max', [
                  button(
                    onClick: () => setState(() => _userModalTab = 'details'),
                    classes: 'px-4 py-1.5 rounded-full text-[10px] font-black transition-all '
                ...
                if (_userModalTab == 'details') ...[
                  div(classes: 'flex flex-col gap-4 text-xs text-zinc-700 pt-2', [
                    div(classes: 'grid grid-cols-2 gap-4', [
                      div(classes: 'bg-[#f8faf9] p-3.5 rounded-2xl border border-zinc-200/20', [
                        span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Contact Email')]),
                        p(classes: 'text-sm font-black text-zinc-900 mt-1', [text(_selectedUser!.email)])
                      ]),
                      div(classes: 'bg-[#f8faf9] p-3.5 rounded-2xl border border-zinc-200/20', [
                        span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Phone Number')]),
                        p(classes: 'text-sm font-black text-zinc-900 mt-1', [text(_selectedUser!.phoneNumber ?? 'N/A')])
                      ]),
                    ]),
                    ...
                    div(classes: 'grid grid-cols-4 gap-3', [
                      div(classes: 'bg-zinc-50/50 p-3 rounded-xl border border-zinc-100 flex flex-col gap-1', [
                        ...
                        span(classes: 'px-2 py-0.5 rounded text-[8px] font-black w-max border '
                          ...
                          text(_selectedUser!.idVerified ? 'YES' : 'NO')
                        ])
                      ]),
                      div(classes: 'bg-zinc-50/50 p-3 rounded-xl border border-zinc-100 flex flex-col gap-1', [
                        span(classes: 'text-[8px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Background Checked')]),
                        span(classes: 'px-2 py-0.5 rounded text-[8px] font-black w-max border '
                            '${_selectedUser!.bgChecked ? "bg-emerald-50 text-[#0fa958] border-emerald-100" : "bg-zinc-100 text-zinc-400 border-zinc-200"}', [
                          text(_selectedUser!.bgChecked ? 'YES' : 'NO')
                        ])
                      ]),
                      div(classes: 'bg-zinc-50/50 p-3 rounded-xl border border-zinc-100 flex flex-col gap-1', [
                    ...
                    div(classes: 'flex flex-col gap-1 border-t border-zinc-100 pt-3', [
                      span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Linked Web3 Solana Wallet Address')]),
                      if (_selectedUser!.walletPublicKey != null && _selectedUser!.walletPublicKey!.isNotEmpty) ...[
                        a(
                          ...
                          [text(_selectedUser!.walletPublicKey!)]
                        ),
                        span(classes: 'text-[8px] text-zinc-400 uppercase font-black ml-1 mt-0.5', [
                          text(_selectedUser!.connectedWalletType ?? 'solana')
                        ])
                      ] else
                    ...
                    div(classes: 'flex flex-wrap gap-4 border-t border-zinc-100 pt-4 mt-2 justify-between items-center', [
                      ...
                      button(
                        onClick: () async {
                          if (web.window.confirm('PERMANENTLY DELETE account for ${_selectedUser!.name}? This cannot be undone.')) {
                            final firestore = context.read(firestoreProvider);
                  ...
                  div(classes: 'flex flex-col gap-3.5 pt-2', [
                    context.watch(userTransactionsProvider(_selectedUser!.uid)).when(
                      data: (txs) {
                        if (txs.isEmpty) {
                          return div(classes: 'py-10 text-center text-xs text-zinc-400 font-semibold', [text('No transactions found for this user.')]);
                        }
                        return div(classes: 'overflow-x-auto w-full rounded-2xl border border-zinc-150', [
                          table(classes: 'w-full text-left text-[11px] border-collapse', [
                            thead(classes: 'bg-zinc-50 border-b border-zinc-150 text-[9px] font-bold text-zinc-400 uppercase tracking-wider', [
                              tr([
                                th(classes: 'p-3.5', [text('Tx Hash')]),
                                th(classes: 'p-3.5', [text('Type')]),
                                th(classes: 'p-3.5 text-center', [text('Status')]),
                                th(classes: 'p-3.5 text-right', [text('Amount')]),
                              ])
                            ]),
                                ...
                                tr(classes: 'hover:bg-zinc-50/50 transition-colors', [
                                  td(classes: 'p-3.5 font-mono text-[9px]', [
                                    a(
                                      ...
                                      events: {'click': (e) => e.stopPropagation()},
                                      [
                                        text(tx.signature.length > 10 
                                            ? '${tx.signature.substring(0, 5)}...${tx.signature.substring(tx.signature.length - 5)}' 
                                            : tx.signature)
                                      ]
                                    )
                                  ]),
                                  td(classes: 'p-3.5 uppercase text-[9px] font-extrabold text-zinc-400', [text(tx.type)]),
                                  td(classes: 'p-3.5 text-center', [
                                    span(classes: 'px-2 py-0.5 rounded text-[8px] font-black border '
                                  ...
                                  td(classes: 'p-3.5 text-right font-black text-zinc-850', [text('₱${tx.amount.toStringAsFixed(2)}')]),
                                ])
                            ])
                          ])
                        ]);
                      },
                      loading: () => div(classes: 'flex justify-center py-10', [div(classes: 'animate-spin h-5 w-5 border-2 border-zinc-200 border-t-indigo-500 rounded-full', [])]),
                      error: (e, _) => div(classes: 'p-4 bg-red-50 text-red-500 rounded-xl text-xs font-mono', [text('Error: $e')]),
                    )
                  ])
                ] else ...[
                  div(classes: 'flex flex-col gap-4 pt-2', [
                    context.watch(userActivityHistoryProvider(_selectedUser!.uid)).when(
                      data: (events) {
                        if (events.isEmpty) {
                          return div(classes: 'py-10 text-center text-xs text-zinc-400 font-semibold', [text('No activities recorded in ledger.')]);
                        }
                        return div(classes: 'flex flex-col gap-3.5 pl-4 border-l border-zinc-250 relative ml-2 text-left', [
                          for (final ev in events)
                            div(classes: 'relative flex flex-col gap-1 pb-1 text-left', [
                              div(classes: 'absolute -left-[21.5px] top-1 w-2.5 h-2.5 rounded-full border-2 border-white bg-indigo-500 shadow-sm', []),
                              div(classes: 'flex justify-between items-start text-[10px] text-left', [
                                span(classes: 'font-black text-zinc-800 leading-snug', [text(ev.title)]),
                                span(classes: 'text-[8px] font-mono text-zinc-400 whitespace-nowrap ml-3', [
                                  text(ev.timestamp > 0 
                                      ? DateTime.fromMillisecondsSinceEpoch(ev.timestamp).toString().split('.').first 
                                      : 'N/A')
                                ])
                              ]),
                              span(classes: 'text-[8px] font-black uppercase tracking-wider text-indigo-400', [text(ev.type.replaceAll('_', ' '))])
                            ])
                        ]);
                      },
                      loading: () => div(classes: 'flex justify-center py-10', [div(classes: 'animate-spin h-5 w-5 border-2 border-zinc-200 border-t-indigo-500 rounded-full', [])]),
      ...
      if (toast != null)
        div(
          classes: 'fixed top-6 right-6 z-50 px-5 py-3 rounded-2xl shadow-xl text-xs font-bold '
              '${toast.startsWith("✅") ? "bg-[#0fa958] text-white" : toast.startsWith("⚠️") ? "bg-amber-500 text-white" : "bg-red-500 text-white"}'
      ...
      div(classes: 'flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-200/50 pb-5', [
        div(classes: 'flex flex-col gap-1', [
          h1(classes: 'text-xl font-black tracking-tight text-zinc-900', [text('User Accounts Directory')]),
          p(classes: 'text-xs text-zinc-400 font-medium', [
            text('Audit user profiles, trust badges, and run background check clearance.'),
          ]),
        ]),
        div(classes: 'flex items-center gap-3', [
          ...
          if (activeTab == 'support' && isAdmin)
            button(
              ...
              classes: 'px-4 py-2.5 bg-black hover:bg-zinc-800 text-white text-xs font-extrabold rounded-full '
                  'shadow-md shadow-black/10 transition-all flex items-center gap-2',
              [
                span(classes: 'text-sm', [text(_showAddAgentForm ? '✕' : '+')]),
                text(_showAddAgentForm ? 'Cancel' : 'Add Agent'),
              ],
            ),
        ]),
      ]),
      ...
      if (_showAddAgentForm && activeTab == 'support')
        div(classes: 'w-full bg-white border border-zinc-200/50 rounded-[24px] p-6 shadow-sm flex flex-col gap-4', [
          div(classes: 'flex flex-col gap-0.5', [
            h3(classes: 'text-sm font-black text-zinc-900', [text('Create Support Agent')]),
            p(classes: 'text-xs text-zinc-400 font-medium', [
              text('New agents will be added to the support queue and visible in Live Customer Service.')
            ]),
          ]),
          div(classes: 'grid grid-cols-1 md:grid-cols-3 gap-3', [
            div(classes: 'flex flex-col gap-1.5', [
              label(classes: 'text-[10px] font-extrabold text-zinc-500 uppercase tracking-wider', [text('Full Name')]),
              input(
                value: _agentName,
                onInput: (v) => setState(() => _agentName = v as String),
                classes: 'px-4 py-2.5 bg-[#f3f6f4] border border-zinc-200 rounded-xl text-xs text-zinc-900 '
                    'focus:outline-none focus:ring-2 focus:ring-black/10',
                attributes: {'placeholder': 'e.g. Juan dela Cruz'},
              ),
            ]),
            div(classes: 'flex flex-col gap-1.5', [
              label(classes: 'text-[10px] font-extrabold text-zinc-500 uppercase tracking-wider', [text('Email Address')]),
              input(
                value: _agentEmail,
            ...
            div(classes: 'flex flex-col gap-1.5', [
              label(classes: 'text-[10px] font-extrabold text-zinc-500 uppercase tracking-wider', [text('Temporary Password')]),
              input(
          ...
          div(classes: 'flex justify-end', [
            button(
              ...
              classes: 'px-6 py-2.5 rounded-xl text-xs font-extrabold transition-all '
              ...
              [
                if (_isCreatingAgent)
                  div(classes: 'flex items-center gap-2', [
                    div(classes: 'w-3.5 h-3.5 border-2 border-zinc-400 border-t-zinc-200 rounded-full animate-spin', []),
                    text('Creating...'),
                  ])
                else
                  text('Create Agent')
        ...
        data: (allUsers) {
          ...
          if (users.isEmpty) {
            return div(
              ...
              [
                span(classes: 'text-3xl mb-3', [text(activeTab == 'support' ? '🛡️' : '👥')]),
                h3(classes: 'text-sm font-bold text-zinc-900', [
                  text(activeTab == 'support' ? 'No support agents found' : 'No accounts found')
                ]),
                p(classes: 'text-xs text-zinc-500 mt-1', [
                  text(activeTab == 'support'
              ...
              [
                table(classes: 'w-full text-left text-xs border-collapse', [
                  thead(
                    ...
                    [
                      tr([
                        th(classes: 'p-5', [text('Agent Name')]),
                        th(classes: 'p-5', [text('Email')]),
                        th(classes: 'p-5 text-center', [text('Role')]),
                        th(classes: 'p-5 text-center', [text('Status')]),
                        th(classes: 'p-5 text-right', [text('Actions')]),
                      ]),
                    ],
                  ),
                  tbody(classes: 'divide-y divide-zinc-50', [
                      ...
                      tr(classes: 'hover:bg-[#fcfdfc] transition-colors', [
                        td(classes: 'p-5 font-bold text-zinc-900', [
                          div(classes: 'flex items-center gap-2.5', [
                            div(classes: 'w-7 h-7 rounded-full bg-indigo-50 border border-indigo-100 flex items-center justify-center text-[10px] font-extrabold text-zinc-700 flex-shrink-0', [
                              text(u.name.length >= 2 ? u.name.substring(0, 2).toUpperCase() : u.name.toUpperCase()),
                            ]),
                            div(classes: 'flex flex-col', [
                              span([text(u.name)]),
                              span(classes: 'text-[9px] text-zinc-400 font-mono', [text('UID: ${u.uid.substring(0, mathMin(u.uid.length, 10))}...')]),
                            ]),
                          ]),
                        ]),
                        td(classes: 'p-5 text-zinc-600 font-medium', [text(u.email)]),
                        td(classes: 'p-5 text-center', [
                          span(classes: 'px-2.5 py-0.5 rounded-full text-[10px] font-extrabold border bg-indigo-50 text-indigo-600 border-indigo-100', [
                            text((u.role ?? 'support').toUpperCase()),
                          ]),
                        ]),
                        td(classes: 'p-5 text-center', [
                          span(classes: 'px-2.5 py-0.5 rounded-full text-[10px] font-extrabold border bg-[#e2f1e9] text-[#0fa958] border-emerald-100', [
                            text('ACTIVE'),
                          ]),
                        ]),
            ...
            [
              table(classes: 'w-full text-left text-xs border-collapse', [
                thead(
                  ...
                  [
                    tr([
                      th(classes: 'p-5', [text('User / ID')]),
                      th(classes: 'p-5', [text('Contact Info')]),
                      th(classes: 'p-5', [text('Web3 Wallet')]),
                      th(classes: 'p-5 text-center', [text('ID Checked')]),
                      th(classes: 'p-5 text-center', [text('Background Checked')]),
                      th(classes: 'p-5 text-center', [text('Verification Level')]),
                      th(classes: 'p-5 text-right', [text('Clearance Control')]),
                    ]),
                  ],
                ),
                tbody(classes: 'divide-y divide-zinc-50', [
                        ...
                        td(classes: 'p-5 font-bold text-zinc-900', [
                          div(classes: 'flex flex-col gap-0.5', [
                            span([
                              text(user.name),
                              if (user.banned)
                                span(classes: 'ml-2 px-1.5 py-0.5 rounded bg-red-100 text-red-500 font-black text-[8px] uppercase tracking-wider', [text('Banned')])
                            ]),
                            span(classes: 'text-[9px] text-zinc-400 font-mono', [text('UID: ${user.uid}')]),
                          ]),
                        ]),
                        td(classes: 'p-5 text-zinc-650 font-medium', [
                          div(classes: 'flex flex-col gap-0.5', [
                            span([text(user.email)]),
                            span(classes: 'text-[10px] text-zinc-400', [text(user.phoneNumber ?? 'No Phone')]),
                          ]),
                        ]),
                        td(classes: 'p-5 text-zinc-650 font-medium', [
                          div(classes: 'flex flex-col gap-0.5', [
                            if (user.walletPublicKey != null && user.walletPublicKey!.isNotEmpty) ...[
                              ...
                              span(classes: 'text-[9px] text-zinc-400 uppercase font-black', [
                                text(user.connectedWalletType ?? 'solana'),
                              ]),
                            ] else ...[
                              span(classes: 'text-zinc-400 font-semibold', [text('Not Linked')]),
                            ],
                          ]),
                        ]),
                        td(classes: 'p-5 text-center', [
                          span(
                            ...
                            [text(user.idVerified ? 'VERIFIED' : 'UNVERIFIED')],
                          ),
                        ]),
                        td(classes: 'p-5 text-center', [
                          span(
                            ...
                            [text(user.bgChecked ? 'CLEARED' : 'UNCLEARED')],
                          ),
                        ]),
                        td(classes: 'p-5 text-center', [
                          span(
                            ...
                            [text('LEVEL ${user.verificationLevel}')],
                          ),
                        ]),
                        td(classes: 'p-5 text-right', [
                          if (!user.bgChecked)
        ...
        div(classes: 'flex flex-col gap-3.5', [
          div(classes: 'flex flex-col gap-1', [
            h3(classes: 'text-sm font-bold text-zinc-900 flex items-center gap-2', [
              span([text('⛓️')]),
              text('Live Blockchain Ledger Transactions'),
            ]),
            p(classes: 'text-xs text-zinc-400 font-semibold', [
              text('Real-time updates of digital wallet escrow and settlement hashes.'),
            ]),
          ]),
            ...
            data: (txs) {
              if (txs.isEmpty) {
                return div(classes: 'p-10 bg-white border border-zinc-200/50 rounded-[28px] text-center shadow-sm', [
                  span(classes: 'text-2xl mb-2', [text('🕸️')]),
                  h4(classes: 'text-xs font-bold text-zinc-800', [text('No transactions detected')]),
                  p(classes: 'text-[10px] text-zinc-400 font-semibold mt-1', [
                    text('Awaiting real-time blocks from the environment database ledger.'),
                  ]),
                ]);
              }
                ...
                [
                  table(classes: 'w-full text-left text-xs border-collapse', [
                    thead(
                      ...
                      [
                        tr([
                          th(classes: 'p-5', [text('Tx Hash / Signature')]),
                          th(classes: 'p-5', [text('Account Owner')]),
                          th(classes: 'p-5', [text('Type')]),
                          th(classes: 'p-5 text-center', [text('Status')]),
                          th(classes: 'p-5 text-right', [text('Amount Value')]),
                        ]),
                      ],
                    ),
                        ...
                        tr(classes: 'hover:bg-[#fcfdfc] transition-colors', [
                          td(classes: 'p-5 font-mono text-[10px]', [
                            if (tx.signature.startsWith('0x') && tx.signature.length <= 14)
                              span(classes: 'text-zinc-500 font-bold', [text(tx.signature)])
                            else
                              a(
                                href: getExplorerUrl(tx.signature, true, currentEnv),
                                classes: 'text-indigo-500 font-bold hover:underline',
                          ...
                          td(classes: 'p-5 text-zinc-700', [
                            span([text('User: ${tx.uid.substring(0, mathMin(tx.uid.length, 12))}')]),
                          ]),
