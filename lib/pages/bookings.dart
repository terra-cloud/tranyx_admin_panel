...
class _BookingsPageState extends State<BookingsPage> {
  ...
  Component build(BuildContext context) {
    ...
    String getDisplayLocation(Map<String, dynamic> raw) {
      final loc = raw['location'] ?? raw['address'] ?? raw['city'] ?? raw['destination'] ?? raw['jobLocation'] ?? raw['province'];
      if (loc is Map) {
    ...
    return div(classes: 'flex-1 p-6 md:p-8 flex flex-col gap-6 max-w-7xl mx-auto w-full bg-[#eff2f0] relative', [
                ...
                div(classes: 'flex items-start justify-between border-b border-zinc-100 pb-4', [
                  div(classes: 'flex items-center gap-3', [
                    ...
                    div(classes: 'flex flex-col', [
                      ...
                      span(classes: 'text-[9px] text-zinc-400 font-mono mt-0.5', [text('Booking ID: ${_detailsModalItem!.id}')]),
                    ])
                  ]),
                  button(
                    onClick: () {
                ...
                div(classes: 'flex flex-col gap-4 text-xs text-zinc-700', [
                  ...
                  div(classes: 'grid grid-cols-2 gap-4', [
                    div(classes: 'bg-[#f8faf9] p-3.5 rounded-2xl border border-zinc-200/20', [
                      span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Deal Parameters')]),
                      p(classes: 'text-sm font-black text-zinc-900 mt-1', [text(_detailsModalItem!.rateLabel)])
                    ]),
                    div(classes: 'bg-[#f8faf9] p-3.5 rounded-2xl border border-zinc-200/20', [
                      span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Transaction Status')]),
                      div(classes: 'mt-1.5', [
                  ...
                  div(classes: 'grid grid-cols-2 gap-4', [
                    div(classes: 'flex flex-col gap-1', [
                      span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Client / Renter')]),
                      p(classes: 'font-bold text-zinc-900', [text(_detailsModalItem!.clientName)]),
                      span(classes: 'text-[9px] text-zinc-400 font-mono', [
                        text('ID: ${_detailsModalItem!.rawData['renteeId'] ?? _detailsModalItem!.rawData['applicantId'] ?? 'N/A'}')
                      ])
                    ]),
                    div(classes: 'flex flex-col gap-1', [
                      span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Provider / Host')]),
                      p(classes: 'font-bold text-zinc-900', [
                        text(_detailsModalItem!.providerName.startsWith('Job #') ? 'Job Listing Host' : _detailsModalItem!.providerName)
                      ]),
                      span(classes: 'text-[9px] text-zinc-400 font-mono', [
                        text('ID: ${_detailsModalItem!.rawData['hostId'] ?? _detailsModalItem!.rawData['jobId'] ?? 'N/A'}')
                      ])
                    ]),
                  ]),
                  ...
                  div(classes: 'flex flex-col gap-2 border-t border-zinc-100 pt-3', [
                    span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Schedule & Timeline')]),
                    div(classes: 'grid grid-cols-2 gap-2 bg-[#f8faf9] p-3 rounded-xl border border-zinc-200/20 text-[11px] font-medium text-zinc-650', [
                      if (activeTab == 'vehicles' || activeTab == 'properties') ...[
                        span([text('Start Date: ${formatDate(_detailsModalItem!.rawData['startDate'] ?? _detailsModalItem!.rawData['rentStart'] ?? 0)}')]),
                        span([text('End Date: ${formatDate(_detailsModalItem!.rawData['endDate'] ?? _detailsModalItem!.rawData['rentEnd'] ?? 0)}')]),
                        span([text('Duration: ${_detailsModalItem!.rawData['multiplier'] ?? 1} ${_detailsModalItem!.rawData['durationType'] ?? 'units'}')]),
                        span([text('Booked At: ${formatDate(_detailsModalItem!.rawData['createdAt'] ?? 0)}')]),
                      ] else ...[
                        span([text('Bid Rate: ₱${(_detailsModalItem!.rawData['proposalRate'] ?? 0).toString()}')]),
                        span([text('Work Estimate: ${_detailsModalItem!.rawData['deliveryTime'] ?? 'N/A'}')]),
                        span([text('Applied At: ${formatDate(_detailsModalItem!.rawData['createdAt'] ?? 0)}')]),
                        span([text('Cover Note: ${_detailsModalItem!.rawData['coverLetter'] ?? 'None provided'}')]),
                      ],
                    ]),
                  ]),
                  ...
                  if (_detailsModalItem!.rawData['signature'] != null || _detailsModalItem!.rawData['txHash'] != null)
                    div(classes: 'flex flex-col gap-1 border-t border-zinc-100 pt-3', [
                      span(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Escrow Transaction Signature')]),
                      p(classes: 'font-mono text-[10px] text-indigo-500 font-bold bg-zinc-50 p-2.5 rounded-lg border border-zinc-200/30 break-all', [
                        text((_detailsModalItem!.rawData['signature'] ?? _detailsModalItem!.rawData['txHash']).toString())
                      ]),
                    ]),
                ]),
      ...
      div(classes: 'flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-200/50 pb-5', [
        div(classes: 'flex flex-col gap-1', [
          h1(classes: 'text-xl font-black tracking-tight text-zinc-900', [text('Bookings & Requests')]),
          p(classes: 'text-xs text-zinc-400 font-medium', [text('Review active transaction agreements and job applications.')]),
        ]),
      ...
      div(classes: 'w-full bg-white border border-zinc-200/50 rounded-[24px] p-5 shadow-[0_4px_20px_rgba(0,0,0,0.01)] flex flex-col gap-4', [
        ...
        div(classes: 'flex items-center justify-between', [
          h3(classes: 'text-[11px] font-extrabold text-zinc-400 uppercase tracking-wider', [text('Booking Parameters Filter')]),
          if (_searchQuery.isNotEmpty || _locationFilter.isNotEmpty || _statusFilter.isNotEmpty || _priceMin != null || _priceMax != null)
            button(
              onClick: () => setState(() {
                _searchQuery = '';
                _locationFilter = '';
        ...
        div(classes: 'grid grid-cols-1 md:grid-cols-5 gap-3.5', [
          ...
          div(classes: 'flex flex-col gap-1.5', [
            label(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wide', [text('Search Keywords')]),
            input(
              value: _searchQuery,
          ...
          // Status Dropdown
          div(classes: 'flex flex-col gap-1.5', [
            label(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wide', [text('Booking Status')]),
            select(
              classes: 'px-4 py-2.5 bg-[#f3f6f4] border border-zinc-200/50 rounded-xl text-xs text-zinc-900 focus:outline-none focus:ring-1 focus:ring-black/10',
              onChange: (v) => setState(() => _statusFilter = v.isNotEmpty ? v.first : ''),
              [
                option(value: '', selected: _statusFilter == '', [text('ALL STATUSES')]),
                option(value: 'pending', selected: _statusFilter == 'pending', [text('PENDING')]),
                option(value: 'approved', selected: _statusFilter == 'approved', [text('APPROVED')]),
                option(value: 'completed', selected: _statusFilter == 'completed', [text('COMPLETED')]),
                option(value: 'cancelled', selected: _statusFilter == 'cancelled', [text('CANCELLED')]),
              ]
            ),
          ]),
          // Min Price
          div(classes: 'flex flex-col gap-1.5', [
            label(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wide', [text('Min Total (₱)')]),
            input(
          ...
          // Max Price
          div(classes: 'flex flex-col gap-1.5', [
            label(classes: 'text-[9px] font-extrabold text-zinc-400 uppercase tracking-wide', [text('Max Total (₱)')]),
            input(
      ...
      bookingsAsync.when(
        data: (bookings) {
          ...
          final filteredBookings = bookings.where((item) {
            ...
            final total = (item.rawData['totalCost'] as num?)?.toDouble() ?? (item.rawData['proposalRate'] as num?)?.toDouble() ?? 0.0;
          ...
          if (filteredBookings.isEmpty) {
            return div(classes: 'flex-grow flex flex-col items-center justify-center text-center p-16 bg-white border border-zinc-200/50 rounded-[28px] shadow-sm', [
              ...
              h3(classes: 'text-sm font-bold text-zinc-900', [text('No bookings match criteria')]),
            ...
            div(classes: 'overflow-x-auto w-full rounded-[28px] border border-zinc-200/50 bg-white shadow-[0_8px_30px_rgba(0,0,0,0.015)]', [
              table(classes: 'w-full text-left text-xs border-collapse', [
                thead(classes: 'bg-[#f8faf9] text-zinc-500 font-bold border-b border-zinc-100 text-[10px] uppercase tracking-wider', [
                  tr([
                    th(classes: 'p-5', [text('Target Asset')]),
                    th(classes: 'p-5', [text('Renter / Applicant')]),
                    th(classes: 'p-5', [text('Provider / Destination')]),
                    th(classes: 'p-5', [text('Location')]),
                    th(classes: 'p-5', [text('Deal Parameters')]),
                    th(classes: 'p-5 text-center', [text('Status')]),
                    th(classes: 'p-5 text-right', [text('Created')]),
                  ])
                ]),
                      ...
                      [
                        td(classes: 'p-5 font-bold text-zinc-900 flex items-center gap-3', [
                          ...
                          div(classes: 'flex flex-col gap-0.5 min-w-0', [
                            span(classes: 'truncate font-bold text-zinc-800 hover:text-indigo-600 transition-colors', [text(item.title)]),
                            span(classes: 'text-[9px] text-zinc-400 font-mono', [text('ID: ${item.id}')]),
                          ])
                        ]),
                        td(classes: 'p-5 text-zinc-650 font-semibold', [text(item.clientName)]),
                        td(classes: 'p-5 text-zinc-650 font-medium', [
                          text(item.providerName.startsWith('Job #') ? 'Job Posting Host' : item.providerName)
                        ]),
                        td(classes: 'p-5 text-zinc-650 font-medium', [text(getDisplayLocation(item.rawData))]),
                        td(classes: 'p-5 font-extrabold text-zinc-900', [text(item.rateLabel)]),
