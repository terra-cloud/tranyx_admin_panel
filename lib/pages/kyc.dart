...
class KycSubmission {
  ...
  factory KycSubmission.fromMap(String uid, Map<String, dynamic> map) {
    ...
    return KycSubmission(
      ...
      status: resStatus,
      submittedAt: parseDateTime(map['submittedAt'] ?? map['createdAt'] ?? idMap?['submittedAt'] ?? bgMap?['submittedAt']),
  ...
  Component build(BuildContext context) {
    ...
    return div(classes: 'flex-grow p-6 md:p-8 flex flex-col gap-6 max-w-7xl mx-auto w-full bg-[#eff2f0] relative', [
        ...
        data: (kycList) {
          ...
          return div(classes: 'flex flex-col gap-4 overflow-y-auto max-h-[600px] pr-2 no-scrollbar', [
            for (final submission in kycList) () {
              final userProfile = users.firstWhere((u) => u.uid == submission.uid, orElse: () => UserProfileModel(uid: submission.uid, name: submission.userName, email: '', idVerified: false, bgChecked: false, verificationLevel: 0, banned: false));
              ...
              return div(
                ...
                [
                  div(
                    events: {
                      'click': (_) => setState(() => _selectedSubmission = submission)
                    },
                    classes: 'flex items-start gap-4 cursor-pointer hover:opacity-85 transition-opacity flex-1 min-w-0',
                    [
                      span(classes: 'text-2xl p-2.5 bg-[#f3f6f4] border border-zinc-200/50 rounded-2xl flex-shrink-0', [
                        text(submission.idType.toLowerCase().contains('passport') ? '🛂' : '🪪'),
                      ]),
                      div(classes: 'flex flex-col gap-1 min-w-0', [
                        span(classes: 'font-bold text-sm text-zinc-900 hover:text-indigo-600 transition-colors flex items-center gap-1.5', [
                          text(resolvedName),
                          span(classes: 'text-[10px] text-indigo-500 font-semibold px-2 py-0.5 rounded-full bg-indigo-50 border border-indigo-100', [text('Inspect File')])
                        ]),
      ...
      if (_selectedSubmission != null) () {
        final userProfile = users.firstWhere((u) => u.uid == _selectedSubmission!.uid, orElse: () => UserProfileModel(uid: _selectedSubmission!.uid, name: _selectedSubmission!.userName, email: '', idVerified: false, bgChecked: false, verificationLevel: 0, banned: false));
        ...
        return div(
          ...
          [
            div(
              classes: 'bg-white rounded-[28px] border border-zinc-200 shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col',
              [
                // Modal Header
                div(classes: 'p-6 border-b border-zinc-150/60 bg-zinc-50 flex justify-between items-center', [
                  div([
                    h3(classes: 'text-sm font-black text-zinc-950 uppercase tracking-wider', [text('Inspection: $resolvedName')]),
                    p(classes: 'text-[11px] text-zinc-400 font-semibold font-mono mt-0.5', [text('UID: ${_selectedSubmission!.uid}')]),
                  ]),
                  button(
                    onClick: () => setState(() => _selectedSubmission = null),
                    classes: 'p-2 text-zinc-400 hover:text-zinc-700 bg-transparent border-0 cursor-pointer text-lg font-bold outline-none',
                    [text('✕')]
                  ),
                ]),
                ...
                div(classes: 'p-6 overflow-y-auto max-h-[500px] flex flex-col gap-6 text-left', [
                  ...
                  div(classes: 'flex flex-col gap-3', [
                    h4(classes: 'text-xs font-black text-zinc-900 uppercase tracking-wide border-l-2 border-indigo-500 pl-2', [text('User Information')]),
                    div(classes: 'grid grid-cols-2 gap-3 text-xs bg-zinc-50 p-4 rounded-xl border border-zinc-200/50', [
                      div([span(classes: 'text-zinc-400 font-bold', [text('Full Name: ')]), text(resolvedName)]),
                      div([span(classes: 'text-zinc-400 font-bold', [text('Email Address: ')]), text(userProfile.email.isNotEmpty ? userProfile.email : 'N/A')]),
                      div([span(classes: 'text-zinc-400 font-bold', [text('Phone Number: ')]), text(userProfile.phoneNumber ?? 'N/A')]),
                      div([span(classes: 'text-zinc-400 font-bold', [text('Verification Level: ')]), text('Level ${userProfile.verificationLevel}')]),
                    ]),
                  ]),

                  // ID Verification Section
                  if (_selectedSubmission!.frontUrl != null || _selectedSubmission!.backUrl != null || _selectedSubmission!.selfieUrl != null || _selectedSubmission!.idNumber != 'N/A')
                    div(classes: 'flex flex-col gap-3', [
                      h4(classes: 'text-xs font-black text-zinc-900 uppercase tracking-wide border-l-2 border-indigo-500 pl-2', [text('Government ID Verification')]),
                      div(classes: 'grid grid-cols-2 gap-2 text-xs bg-zinc-50 p-3 rounded-xl border border-zinc-200/50', [
                        div([span(classes: 'text-zinc-400 font-bold', [text('ID Type: ')]), text(_selectedSubmission!.idType)]),
                        div([span(classes: 'text-zinc-400 font-bold', [text('ID Number: ')]), text(_selectedSubmission!.idNumber)]),
                      ]),
                      if (_selectedSubmission!.frontUrl != null || _selectedSubmission!.backUrl != null || _selectedSubmission!.selfieUrl != null)
                        div(classes: 'grid grid-cols-1 sm:grid-cols-3 gap-4 mt-2', [
                          if (_selectedSubmission!.frontUrl != null)
                            div(classes: 'flex flex-col gap-1.5', [
                              span(classes: 'text-[10px] text-zinc-400 font-bold uppercase tracking-wider', [text('ID Front')]),
                              a(
                                href: _selectedSubmission!.frontUrl!,
                                target: Target.blank,
                                classes: 'block border border-zinc-200 rounded-xl overflow-hidden bg-zinc-50 hover:opacity-95 transition-opacity shadow-sm',
                                [
                                  img(src: _selectedSubmission!.frontUrl!, classes: 'w-full h-32 object-cover bg-zinc-100')
                                ]
                              ),
                            ]),
                          if (_selectedSubmission!.backUrl != null)
                            div(classes: 'flex flex-col gap-1.5', [
                              span(classes: 'text-[10px] text-zinc-400 font-bold uppercase tracking-wider', [text('ID Back')]),
                              a(
                                href: _selectedSubmission!.backUrl!,
                                target: Target.blank,
                                classes: 'block border border-zinc-200 rounded-xl overflow-hidden bg-zinc-50 hover:opacity-95 transition-opacity shadow-sm',
                                [
                                  img(src: _selectedSubmission!.backUrl!, classes: 'w-full h-32 object-cover bg-zinc-100')
                                ]
                              ),
                            ]),
                          if (_selectedSubmission!.selfieUrl != null)
                            div(classes: 'flex flex-col gap-1.5', [
                              span(classes: 'text-[10px] text-zinc-400 font-bold uppercase tracking-wider', [text('Selfie Verification')]),
                              a(
                                href: _selectedSubmission!.selfieUrl!,
                                target: Target.blank,
                                classes: 'block border border-zinc-200 rounded-xl overflow-hidden bg-zinc-50 hover:opacity-95 transition-opacity shadow-sm',
                                [
                                  img(src: _selectedSubmission!.selfieUrl!, classes: 'w-full h-32 object-cover bg-zinc-100')
                                ]
                              ),
                            ]),
                        ]),
                  ...
                  if (_selectedSubmission!.documentUrl != null || _selectedSubmission!.clearanceType != null)
                    div(classes: 'flex flex-col gap-3', [
                      h4(classes: 'text-xs font-black text-zinc-900 uppercase tracking-wide border-l-2 border-indigo-500 pl-2', [text('Background Check Details')]),
                      div(classes: 'grid grid-cols-3 gap-2 text-xs bg-zinc-50 p-3 rounded-xl border border-zinc-200/50', [
                        div([span(classes: 'text-zinc-400 font-bold', [text('Type: ')]), text(_selectedSubmission!.clearanceType ?? 'N/A')]),
                        div([span(classes: 'text-zinc-400 font-bold', [text('Clearance No: ')]), text(_selectedSubmission!.clearanceNumber ?? 'N/A')]),
                        div([span(classes: 'text-zinc-400 font-bold', [text('Expiry: ')]), text(_selectedSubmission!.expiryDate ?? 'N/A')]),
                      ]),
                      if (_selectedSubmission!.documentUrl != null)
                        div(classes: 'flex flex-col gap-1.5 mt-2', [
                          span(classes: 'text-[10px] text-zinc-400 font-bold uppercase tracking-wider', [text('Clearance Document')]),
                          _selectedSubmission!.documentUrl!.toLowerCase().contains('.pdf')
                              ? a(
                                  href: _selectedSubmission!.documentUrl!,
                                  target: Target.blank,
                                  classes: 'inline-flex items-center gap-2 p-3 border border-zinc-200 rounded-xl bg-zinc-50 hover:bg-zinc-100 text-xs font-bold text-indigo-650 transition-all no-underline w-fit',
                                  [
                                    span(classes: 'text-lg', [text('📄')]),
                                    text('View Clearance Document (PDF)')
                                  ]
                                )
                              : a(
                                  href: _selectedSubmission!.documentUrl!,
                                  target: Target.blank,
                                  classes: 'block border border-zinc-200 rounded-xl overflow-hidden bg-zinc-50 hover:opacity-95 transition-opacity max-w-sm shadow-sm',
                                  [
                                    img(src: _selectedSubmission!.documentUrl!, classes: 'w-full h-48 object-contain bg-zinc-100')
                                  ]
                                ),
                        ]),
                    ]),
                ...
                div(classes: 'p-6 border-t border-zinc-150/60 bg-zinc-50 flex justify-end gap-3', [
                  if (_selectedSubmission!.status == 'Pending') ...[
                    button(
                      ...
                      classes: 'px-5 py-2.5 rounded-xl border border-red-200 bg-red-50/50 hover:bg-red-100/50 text-red-500 text-xs font-bold transition-all cursor-pointer outline-none',
                    ...
                    button(
                      ...
                      classes: 'px-5 py-2.5 rounded-xl bg-black hover:bg-zinc-800 text-white text-xs font-bold transition-all shadow-md shadow-black/10 cursor-pointer border-0 outline-none',
                      [text('Approve Verification')]
                    ),
                  ] else ...[
                    button(
                      onClick: () => setState(() => _selectedSubmission = null),
                      classes: 'px-5 py-2.5 rounded-xl bg-zinc-200 hover:bg-zinc-300 text-zinc-800 text-xs font-bold transition-all cursor-pointer border-0 outline-none',
                      [text('Close')]
                    ),
                  ],
                ]),
