...
class _SidebarState extends State<Sidebar> {
  ...
  Component build(BuildContext context) {
    ...
    Component buildMenuItem(String label, String path, String icon) {
      ...
      return li(classes: 'w-full', [
        a(
          ...
          classes: 'flex items-center gap-3.5 px-4 py-3 rounded-2xl text-xs font-bold transition-all duration-300 '
              '${isActive 
          ...
          [
            span(classes: 'text-base flex-shrink-0 flex items-center justify-center w-5 h-5', [
              icon.endsWith('.png')
                  ? img(
                      src: icon,
                      classes: 'w-4.5 h-4.5 object-contain transition-all '
    ...
    Component buildMobileMenuItem(String label, String path, String icon) {
      ...
      return li([
        a(
          ...
          classes: 'flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold '
    ...
    return .fragment([
      ...
      aside(
        classes: 'hidden md:flex flex-col bg-white border-r border-zinc-200/50 p-4 min-h-screen sticky top-0 justify-between items-center transition-all duration-300 '
            '${_isCollapsed ? "w-20" : "w-64"}',
        [
          // Top: Brand logo
          div(classes: 'w-full flex flex-col gap-8', [
            div(classes: 'flex items-center justify-between gap-2.5 w-full '
                '${_isCollapsed ? "justify-center" : "px-2"}', [
              div(classes: 'flex items-center gap-3', [
                span(classes: 'w-10 h-10 rounded-full bg-[#0fa958] text-white flex items-center justify-center font-black text-sm tracking-wider shadow-sm shadow-[#0fa958]/20 flex-shrink-0', [text('T')]),
                if (!_isCollapsed)
                  h2(classes: 'text-sm font-black tracking-wide text-zinc-900 uppercase', [text('Tranyx Admin')]),
              ]),
            ]),
          ...
          div(classes: 'w-full flex flex-col gap-3 pt-4 border-t border-zinc-150/60', [
            ...
            if (!_isCollapsed)
              div(classes: 'px-2 flex items-center gap-3 w-full min-w-0', [
                div(classes: 'w-8 h-8 rounded-full bg-zinc-100 border border-zinc-200 flex items-center justify-center font-bold text-xs text-zinc-700 flex-shrink-0', [
                ...
                div(classes: 'flex flex-col min-w-0 flex-1', [
                  span(classes: 'text-[11px] font-black text-zinc-800 truncate', [
                    text(user?.displayName != null && user!.displayName!.isNotEmpty ? user.displayName! : 'Staff Agent')
                  ]),
                  span(classes: 'text-[9px] text-zinc-400 font-semibold truncate', [text((user as dynamic)?.email ?? 'Staff Agent')]),
                ])
              ]),
            ...
            button(
              ...
              classes: 'flex items-center gap-3.5 px-4 py-3 rounded-2xl text-xs font-bold text-zinc-500 hover:text-red-500 hover:bg-red-50 transition-all duration-300 '
              ...
              [
                span(classes: 'text-base flex-shrink-0', [text('🚪')]),
                if (!_isCollapsed)
                  span([text('LOG OUT')]),
              ],
            ),
            ...
            button(
              ...
              classes: 'flex items-center gap-3.5 px-4 py-2.5 rounded-2xl text-[10px] font-extrabold tracking-wide uppercase text-zinc-400 hover:text-zinc-800 hover:bg-zinc-50 transition-all duration-200 '
                  '${_isCollapsed ? "justify-center px-0 w-11 h-9 mx-auto" : "w-full justify-between"}',
              [
                if (!_isCollapsed) ...[
                  span([text('Collapse Menu')]),
                  span(classes: 'text-xs', [text('◀')]),
                ] else ...[
                  span(classes: 'text-xs', [text('▶')]),
                ]
              ],
      ...
      header(classes: 'flex md:hidden items-center justify-between px-6 py-4 bg-white border-b border-zinc-200 sticky top-0 z-50 shadow-sm', [
        div(classes: 'flex items-center gap-2.5', [
          span(classes: 'w-7.5 h-7.5 rounded-full bg-[#0fa958] text-white flex items-center justify-center font-black text-xs shadow-sm', [text('T')]),
          h2(classes: 'text-xs font-black tracking-wider text-zinc-900 uppercase', [text('Tranyx Admin')]),
        ]),
        button(
          onClick: _toggleMobileMenu,
          classes: 'p-1.5 text-zinc-500 hover:text-zinc-900 focus:outline-none text-lg',
          [text(_isMobileMenuOpen ? '✕' : '☰')],
        )
      ]),
      ...
      if (_isMobileMenuOpen)
        div(
          classes: 'md:hidden fixed inset-x-0 top-[53px] bg-white border-b border-zinc-200 z-40 p-5 flex flex-col gap-5 shadow-2xl transition-all duration-300',
          [
            ...
            div(classes: 'border-t border-zinc-200 pt-4 flex justify-between items-center', [
              span(classes: 'text-[10px] text-zinc-500 font-semibold truncate', [text((user as dynamic)?.email ?? 'Staff Agent')]),
              button(
                onClick: () async {
                  try {
                    web.window.localStorage.removeItem('tranyx_staff_email');
                ...
                classes: 'px-4 py-2 bg-zinc-100 hover:bg-zinc-200 text-zinc-700 hover:text-black rounded-xl text-[9px] font-bold border border-zinc-200 transition-all',
                [text('LOG OUT')],
              )
            ])
          ],
